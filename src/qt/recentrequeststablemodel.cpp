    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return LitecoinFinanceUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, LitecoinFinanceUnits::separatorNever);
            else
                return LitecoinFinanceUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+LitecoinFinanceUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
