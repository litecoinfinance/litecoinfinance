...
WORKINGDIR="/tmp/litecoinfinance_verify_binaries"
TMPFILE="hashes.tmp"
...
BASEDIR="/bin/"
VERSIONPREFIX="litecoinfinance-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'litecoinfinance-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Litecoin Finance Core binary release signing key installed?"
   fi
