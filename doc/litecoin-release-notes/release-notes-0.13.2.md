Litecoin Finance version 0.13.2 is now available from:

  <https://download.litecoinfinance.org/litecoinfinance-0.13.2.1/>
...
ECDSA signatures inside LitecoinFinance transactions now use validation using
...
Previous versions of Litecoin Finance had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Litecoin Finance was to raise the default minimum
relay fee.

Litecoin Finance 0.13.2 will have a strict maximum size on the mempool. The
...
Litecoin Finance 0.13.2 also introduces new default policy limits on the length and
...
Litecoin Finance has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Litecoin Finance will relay transactions with
...
entirely in Litecoin Finance 0.13, and it is at this time undecided whether the
...
Litecoin Finance has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Litecoin Finance automatically creates a hidden service to listen on, without
manual configuration. Litecoin Finance will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Litecoin Finance is listening, and
...
Core to use floating fees. By default, Litecoin Finance will use floating
fees.
...
At all times, Litecoin Finance will cap fees at `-maxtxfee=<x>` (default:
0.10) LTFN.
Furthermore, Litecoin Finance will never create transactions paying less than
...
Various code modernizations have been done. The Litecoin Finance code base has
...
- `litecoinfinance-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `litecoinfinance-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the LitecoinFinance scripting system that in combination with
...
LitecoinFinance transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Litecoin Finance.
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming LitecoinFinance wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each LitecoinFinance block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of litecoinfinances they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of litecoinfinances being spent is exactly
...
- **Increased security for multisig:** LitecoinFinance addresses (both P2PKH addresses
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  note that Litecoin Finance does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  LitecoinFinance users to individually opt-in to almost any change in the LitecoinFinance
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Litecoin Finance contributors that may
...
segwit's start date of 1 January 2017 miners can update the LitecoinFinance
client to Litecoin Finance 0.13.2 to signal for segwit support. When a
...
Since Litecoin Finance 0.10.0, nodes have defaulted to only relaying and
